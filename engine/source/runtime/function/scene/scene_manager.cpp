...
// #include "runtime/resource/res_type/common/mesh_data.h"
...
namespace VE
{
    ...
    public:
        static void clear(Scene* curscene)
        {
            ...
            {
                // for (RenderMesh& mesh : curscene->getMeshs())
                // {
    ...
    MeshHandle SceneBuilder::loadMesh(const std::string& filename, Vector3& min_bound, Vector3& max_bound)
    {
        ...
        std::vector<Mesh_PosNormalTangentTex_Vertex> mesh_vertices;

        for (size_t s = 0; s < shapes.size(); s++)
        {
            size_t index_offset = 0;
            for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) // num_face_vertices-face indice 
            {
                // vertex size of facexxx
                ...
                {
                    Mesh_PosNormalTangentTex_Vertex mesh_vert{};
        ...
        // take care of the index range, should be consistent with the index range used by vulkan
        assert(mesh_vertices.size() <= std::numeric_limits<uint16_t>::max());

        for (size_t i = 0; i < mesh_vertices.size(); i++)
        {
            ((Mesh_PosNormalTangentTex_Vertex*)(vertexMem->m_data))[i] = mesh_vertices[i];
    ...
    TextureHandle SceneBuilder::loadTexture(const char* file, bool sRGB)
    {
        ...
        if (!texture->m_pixels)
            return VIRTUAL_INVALID_HANDLE;

        texture->m_width = iw;
        texture->m_height = ih;
        texture->m_format = (sRGB) ? VIRTUAL_PIXEL_FORMAT::VIRTUAL_PIXEL_FORMAT_R8G8B8_SRGB :
                                     VIRTUAL_PIXEL_FORMAT::VIRTUAL_PIXEL_FORMAT_R8G8B8_UNORM;
        texture->m_depth = 1;
        texture->m_array_layers = 1;
        texture->m_mip_levels = 1;
        texture->m_type = VIRTUAL_IMAGE_TYPE::VIRTUAL_IMAGE_TYPE_2D;
    ...
    void SceneManager::syncSceneObjects()
    {
        ...
        m_mesh_id_gobject_id_map.clear();
        ...
        for (const auto& go_desc : m_go_descs)
        {
            for (size_t i = 0; i < go_desc.getComponents().size(); i++)
            {
                ...
                m_mesh_id_gobject_id_map[mesh.m_instance_id] = go_desc.getId();
                                                        ...
                                                        SkeletonBindingBufferHandle& skeleton_binding_handle)
    {
        ...
        {
            // vertex_buffer 是在json文件中被写入？
            size_t vertex_size = bind_data->vertex_buffer.size() * sizeof(Mesh_PosNormalTangentTex_Vertex);
            const SceneMemory* vertex_data = SceneBuffers::alloc(vertex_size);

            Mesh_PosNormalTangentTex_Vertex* vertex = (Mesh_PosNormalTangentTex_Vertex*)vertex_data->m_data;
        ...
        else
        {
            ...
            Mesh_PosNormalTangentTex_Vertex* vertex = 
                (Mesh_PosNormalTangentTex_Vertex*)((SceneMemory*)vertex_handle.handle)->m_data;

            for (size_t i = 0; i < ((SceneMemory*)vertex_handle.handle)->m_size / sizeof(Mesh_PosNormalTangentTex_Vertex); i++)
            {
                auto x = vertex[i].x;
                auto y = vertex[i].y;
                auto z = vertex[i].z;
        ...
        if (find_it == m_skeleton_binding_handle_map.end())
        {
            ...
            m_skeleton_binding_handle_map.insert(std::make_pair(skeleton_binding_file, SkeletonMeshBinding{skeleton_binding_handle, bounding_box}));
