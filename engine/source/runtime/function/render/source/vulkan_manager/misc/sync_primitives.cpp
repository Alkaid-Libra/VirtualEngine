...
bool VE::VVulkanManager::createSyncPrimitives()
{
    ...
    {
        if (vkCreateSemaphore(m_vulkan_context._device, &semaphore_create_info, nullptr, 
                &m_image_available_for_render_semaphores[i]) != VK_SUCCESS ||
            vkCreateSemaphore(m_vulkan_context._device, &semaphore_create_info, nullptr, 
                &m_image_finished_for_presentation_semaphores[i]) != VK_SUCCESS ||
            vkCreateFence(m_vulkan_context._device, &fence_create_info, nullptr,
                &m_is_frame_in_flight_fences[i]) != VK_SUCCESS)
        {
