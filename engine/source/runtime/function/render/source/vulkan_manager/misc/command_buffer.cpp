...
bool VE::VVulkanManager::initializeCommandPool()
{
    ...
    {
        if (vkCreateCommandPool(m_vulkan_context._device, &command_pool_create_info, NULL, &m_command_pools[i]) != VK_SUCCESS)
        {
...
bool VE::VVulkanManager::initializeCommandBuffers()
{
    ...
    for (uint32_t i = 0; i < m_max_frames_in_flight; ++i)
    {
        ...
        if (vkAllocateCommandBuffers(m_vulkan_context._device, &command_buffer_allocate_info, &m_command_buffers[i]) != VK_SUCCESS)
        {
...
VkCommandBuffer VE::VVulkanManager::getCurrentCommandBuffer() { return m_command_buffers[m_current_frame_index]; }