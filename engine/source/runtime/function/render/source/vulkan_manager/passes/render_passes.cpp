...
bool VE::VVulkanManager::initializeRenderPass()
{
    VRenderPassHelperInfo helper_info{};
    helper_info.v_context = &m_vulkan_context;
    helper_info.descriptor_pool = m_descriptor_pool;
    helper_info.v_global_render_resource = &m_global_render_resource;

    VLightingPass::setContext(helper_info);

    // // initialize before lighting pass
    // m_point_light_pass.initialize();
    // m_directional_light_pass.initialize();

    VLightPassHelperInfo light_pass_helper_info{};
    ...
    // m_point_light_pass._per_mesh_layout       = descriptor_layouts[VLightingPass::LayoutType::_per_mesh];
    // m_directional_light_pass._per_mesh_layout = descriptor_layouts[VLightingPass::LayoutType::_per_mesh];
