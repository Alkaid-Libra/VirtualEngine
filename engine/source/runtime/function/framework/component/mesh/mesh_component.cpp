...
namespace VE
{
    // load wall.mesh.json for example
    MeshComponent::MeshComponent(const MeshComponentRes& mesh_ast, GObject* )
    {
        ...
        for (const SubMeshRes& sub_mesh : mesh_ast.m_sub_meshes)
        {
            ...
            if (meshComponent.material_desc.with_texture)
            {
                ...
                // read material_path(gold.material.json) as material_res
    ...
    void MeshComponent::tick(float delta_time)
    {
        ...
        VE::SkeletonAnimationResult animResult;
        ...
        if (animation_component != nullptr)
        {
            ...
            {
                VE::SkeletonAnimationResultTransform tmp{Matrix4x4(node.tranform)};
