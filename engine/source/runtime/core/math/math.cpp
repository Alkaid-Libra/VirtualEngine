...
namespace VE
{
    ...
    Math::makeViewMatrix(const Vector3& position, const Quaternion& orientation, const Matrix4x4* reflect_matrix)
    {
        ...
        // this is the most efficiently done using 3x3 Matrices
        Matrix3x3 rot;
    ...
    Matrix4x4
    Math::makeLookAtMatrix(const Vector3& eye_position, const Vector3& target_position, const Vector3& up_dir)
    {
    ...
    Matrix4x4 makePerspectiveMatrix(Radian fovy, float aspect, float znear, float zfar)
    {
        ...
        // NDC-[0,1] Direct/Vulkan
        //*NDC-[-1,1] OpenGL (annotation)
    ...
    makeOrthographicProjectionMatrix(float left, float right, float bottom, float top, float znear, float zfar)
    {
        ...
        float A = 2.0f * inv_width;
        float B = 2.0f * inv_height;
