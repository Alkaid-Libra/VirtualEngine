...
namespace VE
{
    ...
    public:
        ...
        float operator[](size_t i) const // can be used by const vector2, and don't modify value
        {
            assert(i < 2);
            return (i == 0 ? x : y);
        }
        ...
        friend Vector2 operator*(float scalar, const Vector2& rhs) { return Vector2(scalar * rhs.x, scalar * rhs.y); } // complete float * v, not v * float
